...
from ddpg import DDPG_Actor, DDPG_Critic
...
class Agent():
    ...
    def __init__(self, is_training, endless, continue_training, render, use_gpu, hyperparameter_set):
        ...
        self.noise_scale            = hyperparameters['noise_scale'] # Changed to noise_scale for simplicity
        ...
        self.actor = DDPG_Actor(self.num_states, self.num_actions, use_gpu, self.action_low, self.action_high).to(self.device)
        self.actor_target = DDPG_Actor(self.num_states, self.action_dim, use_gpu, self.action_low, self.action_high).to(self.device)
        self.actor_target.load_state_dict(self.actor.state_dict())
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=self.learning_rate)

        self.critic = DDPG_Critic(self.num_states, self.num_actions, use_gpu).to(self.device)
        self.critic_target = DDPG_Critic(self.num_states, self.num_actions, use_gpu).to(self.device)
    ...
    def train(self):
        ...
        with torch.no_grad():
            next_action = self.actor_target(next_state).clamp(-self.action_high, self.action_high)
    ...
    def save_graph(self, rewards_per_episode):
        ...
        plt.title(f'DDPG implementation in {self.env_id}')
...
if __name__ == '__main__':
    ...
    DDPG = Agent(args.train, args.endless, args.continue_training, args.render, args.use_gpu, hyperparameter_set=args.hyperparameters)
    DDPG.run(args.train, args.continue_training)
